...
public class StreamPumper implements Runnable {
   ...
   * 
   * <p>
   * <b>Note:</b> If you set useAvailable to true, you must explicitly invoke
   * {@link #stop stop} or interrupt the corresponding Thread when you are done
   * or the run method will never finish on some JVMs (namely those where
   * available returns 0 on a closed stream). Setting it to true may also impact
   * performance negatively. This flag should only be set to true if you intend
   * to stop the pumper before the input stream gets closed.
   * </p>
   * 
   * @param is
   *          input stream to read data from
   ...
   *          exhausted.
   * @param useAvailable
   *          whether the pumper should use
   *          {@link java.io.InputStream#available available} to determine
   *          whether input is ready, thus trying to emulate non-blocking
   *          behavior.
   * 
   * @since Ant 1.8.0
   */
  ...
  /**
   * Stop the pumper as soon as possible. Note that it may continue to block on
   * the input stream but it will really stop the thread as soon as it gets EOF
  ...
  private void waitForInput(final InputStream is)
